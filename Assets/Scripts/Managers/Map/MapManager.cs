...
namespace Gameplay.Map
{
    public class MapManager : Singleton<MapManager>
    {
        ...
        public int GetDepth(int x, int y)
        {
            if (Contains(x, y))
            {
                return tiles.GetDepth(WorldToMapX(x), WorldToMapY(y));
            }
            return 0;
        }
        ...
        public void GizmoDrawRegion(int x, int y, Region region)
        {
            ...
            if (quad != null)
            {
                Gizmos.DrawWireCube(new Vector3(x + quad.branchSize + mapOffset.x - 0.5f, y + quad.branchSize + mapOffset.y - 0.5f, 0), 2 * quad.branchSize * Vector3.one);
                for (int i = 0; i < 2; i++)
                {
                    for (int j = 0; j < 2; j++)
                    {
            ...
            {
                Gizmos.DrawWireCube(new Vector3(x + mapOffset.x, y + mapOffset.y, 0), Vector3.one);
            }
        }
